# Contributing to Payroll Savings Calculator
...
```bash
git clone https://github.com/crmleaf/savings-calculator.git
...
requires. `Crmleaf\Payroll\Calculators\SavingsCalculator` is the class doing the work; everything else
...
Made in the monorepo, against `tools/savings-calculator/`:
...
```
fix(savings): <what changed>
test(savings): <the case now covered>
```
