...
<head>
    ...
    <title>{{ $title ?? 'Payroll Savings Calculator' }}</title>
    <meta name="description" content="Estimate annual savings against another payroll provider, per employee.">
    <style>
...
<x-crmleaf::savings-calculator
    ...
    :result="$result ?? null"
    :error="$error ?? null"
    :heading="$title ?? 'Payroll Savings Calculator'"
/>
...
<footer class="crmleaf-tool__colophon">
    ...
    <p>
        Computed by <a href="https://github.com/crmleaf/savings-calculator">crmleaf/savings-calculator</a>,
...
{{--
    ...
    The bundled script binds any element carrying data-crmleaf-tool="savings-calculator",
    computes on submit, and only falls back to posting the form if the browser
    build is not present.
--}}
@if (config('savings-calculator.assets.script', true))
    @if (config('savings-calculator.assets.cdn'))
        <script src="{{ config('savings-calculator.assets.cdn') }}" defer></script>
    @endif
    <script src="{{ asset('vendor/savings-calculator/savings-calculator.js') }}" defer></script>
@endif
